...
def _print_welcome() -> None:
    ...
    tips = Text()
    tips.append("\n  直接输入自然语言，AI Agent 自动执行\n", style="bold white")
    tips.append("  或使用斜杠命令: ", style="dim")
...
def _run_agent_task(goal: str) -> None:
    """在 REPL 中执行自然语言任务，带实时流式进度"""
    from .agent_loop import AgentLoop

    console.print()
    console.print(f"  [dim]任务: {goal}[/dim]")

    # 用简单变量追踪 spinner 状态，避免 Rich 内部 API 兼容问题
    spinner_active = [True]  # 用 list 以便闭包修改
    ...
    def on_step(step):
        ...
        icon = "[green][OK][/green]" if step.result_ok else "[red][FAIL][/red]"
        console.print(
            f"  {icon} [bold]{step.tool_name}[/bold] "
            f"[dim]({step.duration_ms:.0f}ms)[/dim]"
        )
        if not step.result_ok and step.result_error:
            console.print(f"    [red dim]{step.result_error[:150]}[/red dim]")
    ...
    # 显示总结
    if result.success:
        console.print(f"  [bold green]完成[/bold green] ", end="")
    else:
        console.print(f"  [bold yellow]未完全完成[/bold yellow] ", end="")
    console.print(f"[dim]{result.total_duration_ms:.0f}ms | {result.step_count} 步[/dim]")
    if result.final_summary:
        console.print(f"  [white]{result.final_summary[:500]}[/white]")
    elif result.error:
        console.print(f"  [red dim]{result.error[:200]}[/red dim]")
